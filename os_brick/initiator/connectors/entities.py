...
class StorageClass(JsonClass):
    ...
    def __init__(
        ...
        if maxBWPerGB is not None:
            self.maxBWPerGB = maxBWPerGB * 1024
        if desiredBWPerGB is not None:
            self.desiredBWPerGB = desiredBWPerGB * 1024
...
class AuthServerEntity(JsonClass):
    #
    # Provisioner Authorization Sefver Entity data
    #

    def __init__(self, publicKey, authorizationServerJwkSetURL,
                 tokenURL, provisionerResourceId, provisionerClientId,
                 provisionerClientSecret, provisionerClientScope,
                 backendsResourceId, backendsClientId,
                 backendsClientSecret, backendsClientScope):
        self.authorizationServerPublicKey = publicKey
        self.authorizationServerJwkSetURL = authorizationServerJwkSetURL
        self.authorizationServerTokenURL = tokenURL
        self.provisionerResourceID = provisionerResourceId
        self.provisionerClientID = provisionerClientId
        self.provisionerClientSecret = provisionerClientSecret
        self.provisionerClientScope = provisionerClientScope
        self.backendsResourceID = backendsResourceId
        self.backendsClientID = backendsClientId
        self.backendsClientSecret = backendsClientSecret
        self.backendsClientScope = backendsClientScope
        
class TaskEntity(JsonClass):
    #
    # Provisioner Task Entity data for Show operation
    #

    def __init__(self, state, startDate, endDate, progress, type,
                 statusDescription, stoppable, taskId, taskConfiguration,
                 hostId, taskStatus):
        self.state = state
...
class TagEntity(JsonClass):
    #
    # Provisioner Tag Entity data for Show operation
    #
